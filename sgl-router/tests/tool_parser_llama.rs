...
use vllm_router_rs::tool_parser::{LlamaParser, ToolParser};
...
async fn test_llama_streaming_simple() {
    let parser = LlamaParser::new();
    let mut state = vllm_router_rs::tool_parser::ParseState::new();
    ...
    match result {
        vllm_router_rs::tool_parser::StreamResult::ToolComplete(tool) => {
...
async fn test_llama_streaming_partial() {
    ...
    for chunk in chunks {
        ...
        if let vllm_router_rs::tool_parser::StreamResult::ToolComplete(tool) = result {
...
async fn test_llama_streaming_multiple_tools_chunked() {
    ...
    match result1 {
        vllm_router_rs::tool_parser::StreamResult::Incomplete
        | vllm_router_rs::tool_parser::StreamResult::ToolName { .. }
        | vllm_router_rs::tool_parser::StreamResult::ToolArguments { .. } => {
