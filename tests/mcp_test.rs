...
// functionality required for vLLM responses API integration.
//
// Test Coverage:
...
use serde_json::json;
use vllm_router_rs::mcp::{parse_sse_event, MCPToolServer, MultiToolSessionManager, ToolSession};
...
async fn test_responses_api_integration_patterns() {
    ...
    let result = mcp_server
        .call_tool(
            "brave_web_search",
            json!({
                "query": "vLLM router MCP integration",
                "count": 1
            }),
        )
        .await;
